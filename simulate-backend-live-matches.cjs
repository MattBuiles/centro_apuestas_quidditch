...
console.log('🔍 Simulando partidos en vivo en la base de datos del backend...\n');
console.log('📂 Usando base de datos:', dbPath);

// Encontrar y simular todos los partidos en vivo
db.all(`
  SELECT 
    m.id,
    m.status,
...
`, (err, liveMatches) => {
  ...
  if (liveMatches.length === 0) {
    console.log('✅ No hay partidos en vivo en la base de datos del backend');
    db.close();
    return;
  }
