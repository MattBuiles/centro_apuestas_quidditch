...
// Abrir la base de datos del backend
const dbPath = path.join(__dirname, 'database', 'quidditch.db');
const db = new sqlite3.Database(dbPath);

console.log('🔍 Verificando partidos en vivo en la base de datos del backend...\n');
...
`, (err, liveMatches) => {
  ...
  if (liveMatches.length > 0) {
    ...
    // Simular automáticamente el primer partido en vivo
